...
- **חוויית קריאה מלאה**: dropdown של פרקים במובייל, TOC צדדי בדסקטופ, זמן קריאה משוער, עיגול התקדמות, חגיגה בסיום (קונפטי + בלונים + צלצול) — חד-פעמי, לא חוזר ברענון
...
| `/y/:year/:slug` | תצוגת טקסט של עלון (כולל likes + share-cta בסוף) |
...
| מפתח | תוכן |
|---|---|
| `mashmaut.fp` | UUID לזיהוי דפדפן (משותף לאנליטיקס + לייקים) |
...
3. **`public/data/config.json`** — תצורת האתר (כולל `dispatchSchedule` + `logo`)
