...
import { getDisplayName, setDisplayName, promptForDisplayName, setThreadName } from '../lib/displayName.js';
...
export async function renderDiscussNew({ params }) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // A new thread starts a fresh conversation, so the current global name
    // applies (and gets locked to the thread once it's created below).
    ...
    try {
      ...
      // Lock this name to the new conversation.
      setThreadName(r.thread.id, r.thread.author || name);
